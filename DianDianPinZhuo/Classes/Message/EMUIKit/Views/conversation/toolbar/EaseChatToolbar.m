...
- (void)_setupSubviews
{
    ...
    [styleChangeButton setImage:[UIImage imageNamed:@"EaseUIResource.bundle/chatBar_recordSelected"] forState:UIControlStateHighlighted];
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    if ([text isEqualToString:@"\n"]) {
        if ([self.delegate respondsToSelector:@selector(didSendText:withExt:)]) {
            [self.delegate didSendText:textView.text withExt:@{@"userName":[HQDefaultTool getNickName],@"icon":[HQDefaultTool getHead],@"type":@"0",@"content":@"",@"confirm":@"",@"webTitle":@"",@"webUrl":@""}];
    ...
    if (chatText.length > 0) {
        if ([self.delegate respondsToSelector:@selector(didSendText:withExt:)]) {
            ...
            if (![_inputTextView.text isEqualToString:@""]) {
                ...
                [self.delegate didSendText:attStr withExt:@{@"userName":[HQDefaultTool getNickName],@"icon":[HQDefaultTool getHead],@"type":@"0",@"content":@"",@"confirm":@"",@"webTitle":@"",@"webUrl":@""}];
                self.inputTextView.text = @"";
                [self _willShowInputTextViewToHeight:[self _getTextViewContentH:self.inputTextView]];;
            }
        }
