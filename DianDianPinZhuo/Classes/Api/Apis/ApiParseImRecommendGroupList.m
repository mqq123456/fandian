//
//  ApiParseImRecommendGroupList.m
//  DianDianPinZhuo
//
...
@implementation ApiParseImRecommendGroupList
-(RequestModel *)requestData:(ReqImRecommendGroupList *)reqModel
{
    ...
    requestModel.url=[NSString stringWithFormat:@"%@%@",self.url,API_IM_RECOMMEND_GROUP_LIST];
    requestModel.parameters=self.params;
    
    MQQLog(@"ReqImRecommendGroupList=%@",[self.datas description]);
    return requestModel;
}

-(RespImRecommendGroupList *)parseData:(id)resultData
{
    RespImRecommendGroupList *respModel=[[RespImRecommendGroupList alloc]init];
    if (resultData!=nil) {
    ...
    MQQLog(@"RespImRecommendGroupList=%@",[resultData description]);
