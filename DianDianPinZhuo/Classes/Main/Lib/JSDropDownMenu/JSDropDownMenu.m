...
@property (nonatomic, weak) UIImageView *bottonImage;
@end
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    [self.bottonImage removeFromSuperview];
    UIImageView *imageView = [[UIImageView alloc] init];
    [self addSubview:imageView];
    imageView.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 0);
    imageView.image = [UIImage imageNamed:@"content_img_shoubing_nor"];
    self.bottonImage = imageView;
...
- (void)animateLeftTableView:(UITableView *)leftTableView rightTableView:(UITableView *)rightTableView show:(BOOL)show complete:(void(^)())complete {
        ...
        if (IPHONE4) {
            [UIView animateWithDuration:0.2 animations:^{
                self.bottonImage.frame = CGRectMake(0, 44+(38*[_leftTableView numberOfRowsInSection:0])-10, [UIScreen mainScreen].bounds.size.width, 20);
            }];
        }else if (IPHONE5){
            [UIView animateWithDuration:0.2 animations:^{
                self.bottonImage.frame = CGRectMake(0, 38+(40*[_leftTableView numberOfRowsInSection:0]), [UIScreen mainScreen].bounds.size.width, 20);
            }];
        }else if(IPhone6||IPhone6Plus) {
            [UIView animateWithDuration:0.2 animations:^{
                self.bottonImage.frame = CGRectMake(0, self.frame.size.height+(44*[_leftTableView numberOfRowsInSection:0]), [UIScreen mainScreen].bounds.size.width, 20);
            }];
        }
    ...
    if (show) {
        ...
        if (leftTableView) {
            ...
            leftTableViewHeight = ([leftTableView numberOfRowsInSection:0]*_leftTableView.rowHeight);
//            if ([rightTableView numberOfRowsInSection:0] ==1) {
//                leftTableViewHeight = 9 * leftTableView.rowHeight;
//            }
        ...
        if (rightTableView) {
            ...
            rightTableViewHeight = ([rightTableView numberOfRowsInSection:0] > 9) ? (9 * rightTableView.rowHeight) : ([rightTableView numberOfRowsInSection:0] * rightTableView.rowHeight);
        }
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [UIView animateWithDuration:0.2 animations:^{
                self.bottonImage.frame = CGRectMake(0, 40, [UIScreen mainScreen].bounds.size.width, 0);
            }];
            
            if ([rightTableView numberOfRowsInSection:0]==9||[rightTableView numberOfRowsInSection:0]==1) {
...
//    UILabel *titleLabel = (UILabel*)[cell viewWithTag:1];
    ...
    if (leftOrRight == 1) {
        ...
        CGFloat marginX = [UIScreen mainScreen].bounds.size.width/4-(textSize.width/2);
        ...
        if ([titleLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
            ...
            UIImageView *accessoryImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"order_ico_yuan_dis"]];
            
            accessoryImageView.frame = CGRectMake(titleLabel.frame.origin.x-22, (self.frame.size.height-12)/2, 12, 12);
        ...
        if (!_hadSelected && _leftSelectedRow == indexPath.row) {
            ...
            if (indexPath.row < self.best_select_index-1&&[tableView numberOfRowsInSection:0] !=7) {
...
//    if (indexPath.row<self.best_select_index-1) {
//
//        return;
//    }
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([cell.textLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
        cell.backgroundColor = BackColor;
        cell.accessoryView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"order_ico_yuan_dis"]];
