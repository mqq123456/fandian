            ...
            didSelectConversationModel:(id<IConversationModel>)conversationModel
{
    if (conversationModel) {
        ...
        if (conversation) {
            ...
            chatController.content = self.share_title;
            chatController.webTitle = self.share_hint;
#warning 这里的share_url 赋给了 webUrl
            chatController.webUrl = self.share_url;
            chatController.share = self.share;
            chatController.type = self.type;
            chatController.share_hint =self.share_hint;
            if (conversation.conversationType==eConversationTypeChat) {
                ...
                chatController.is_join_in = YES;//用户加入过的群组，删掉应用后进入后不会再次发送个人名片
