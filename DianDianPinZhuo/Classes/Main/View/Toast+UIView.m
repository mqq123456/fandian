...
static const NSString * CSMessageToastActivityViewKey = @"CSMessageToastActivityViewKey";
...
- (void)makeMessageToastActivity:(NSString *)message position:(id)point animation:(BOOL)animation
{
    UIView *existingActivityView = (UIView *)objc_getAssociatedObject(self, &CSMessageToastActivityViewKey);
    ...
    UIView *toast = [self viewForMessage:message title:nil image:nil];
    toast.center = [self centerPointForPosition:point ? point : CSToastDefaultPosition withToast:toast];
    toast.alpha = 0.0;
    
    objc_setAssociatedObject (self, &CSMessageToastActivityViewKey, toast, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)hideMessageToastActivity:(BOOL)animation
{
    UIView *existingActivityView = (UIView *)objc_getAssociatedObject(self, &CSMessageToastActivityViewKey);
    if (existingActivityView != nil) {
        if (animation)
        {
            [UIView animateWithDuration:CSToastFadeDuration
                             ...
                             animations:^{
                                 ...
                                 objc_setAssociatedObject (self, &CSMessageToastActivityViewKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
                             }];
        }
        else
        {
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame) {
            ...
            return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height + self.bounds.origin.y - (toast.frame.size.height / 2)) - CSToastVerticalPadding-50);
        } else if([point caseInsensitiveCompare:@"center"] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2 + self.bounds.origin.y / 2);
        }
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (CSToastDisplayShadow) {
        wrapperView.layer.shadowColor = [UIColor colorWithRed:21.0/255.0 green:21.0/255.0 blue:21.0/255.0 alpha:1].CGColor;
        wrapperView.layer.shadowOpacity = CSToastShadowOpacity;
        wrapperView.layer.shadowRadius = CSToastShadowRadius;
        wrapperView.layer.shadowOffset = CSToastShadowOffset;
    }

    wrapperView.backgroundColor = [[UIColor colorWithRed:21.0/255.0 green:21.0/255.0 blue:21.0/255.0 alpha:1] colorWithAlphaComponent:CSToastOpacity];
