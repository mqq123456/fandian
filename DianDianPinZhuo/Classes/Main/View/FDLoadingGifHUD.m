...
//
//  https://github.com/samvermette/FDLoadingGifHUD
//

#if !__has_feature(objc_arc)
#error FDLoadingGifHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "FDLoadingGifHUD.h"
#import <QuartzCore/QuartzCore.h>
#import "UIImage+animatedGIF.h"
#import "UIView+Extension.h"

NSString * const FDLoadingGifHUDDidReceiveTouchEventNotification = @"FDLoadingGifHUDDidReceiveTouchEventNotification";
NSString * const FDLoadingGifHUDWillDisappearNotification = @"FDLoadingGifHUDWillDisappearNotification";
NSString * const FDLoadingGifHUDDidDisappearNotification = @"FDLoadingGifHUDDidDisappearNotification";

NSString * const FDLoadingGifHUDStatusUserInfoKey = @"FDLoadingGifHUDStatusUserInfoKey";

CGFloat FDLoadingGifHUDRingRadius = 14;
CGFloat FDLoadingGifHUDRingThickness = 6;

@interface FDLoadingGifHUD ()

@property (nonatomic, readwrite) FDLoadingGifHUDMaskType maskType;
...
- (void)showProgress:(float)progress
            ...
            maskType:(FDLoadingGifHUDMaskType)hudMaskType;
...
@implementation FDLoadingGifHUD
...
+ (FDLoadingGifHUD*)sharedView {
    static dispatch_once_t once;
    static FDLoadingGifHUD *sharedView;
...
+ (void)show {
    [[self sharedView] showProgress:-1 status:nil maskType:FDLoadingGifHUDMaskTypeNone];
}

+ (void)showWithStatus:(NSString *)status {
    [[self sharedView] showProgress:-1 status:status maskType:FDLoadingGifHUDMaskTypeNone];
}
...
+ (void)showProgress:(CGFloat)progress {
    [[self sharedView] showProgress:progress status:nil maskType:FDLoadingGifHUDMaskTypeNone];
}

+ (void)showProgress:(CGFloat)progress status:(NSString *)status {
    [[self sharedView] showProgress:progress status:status maskType:FDLoadingGifHUDMaskTypeNone];
}
...
+ (void)showImage:(UIImage *)image status:(NSString *)string {
    NSTimeInterval displayInterval = [[FDLoadingGifHUD sharedView] displayDurationForString:string];
    [[self sharedView] showImage:image status:string duration:displayInterval];
}

+ (void)showLoadingGifHUD{
    [self showImage:[[self sharedView] hudErrorImage] status:@""];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    switch (self.maskType) {
        ...
        case FDLoadingGifHUDMaskTypeBlack: {
            [[UIColor colorWithWhite:0 alpha:0.5] set];
            CGContextFillRect(context, self.bounds);
            break;
        }
            
        case FDLoadingGifHUDMaskTypeGradient: {
...
//    self.hudView.layer.borderColor = [UIColor colorWithRed:250.0/255.0 green:250.0/255.0 blue:250.0/255.0 alpha:1].CGColor;
//    self.hudView.layer.borderWidth = 0.5;
    
    //self.imageView.center = CGPointMake(CGRectGetWidth(self.hudView.bounds)/2,  CGRectGetHeight(self.hudView.bounds)/2-22);
    self.imageView.x = hudWidth/2-32;
    self.imageView.y = hudHeight/2-22;
    self.imageView.width = 64;
    self.imageView.height = 44;
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    if(notification) {
        FDLoadingGifHUD *__weak weakSelf=self;
...
- (void)overlayViewDidReceiveTouchEvent {
    [[NSNotificationCenter defaultCenter] postNotificationName:FDLoadingGifHUDDidReceiveTouchEventNotification object:nil];
}

#pragma mark - Master show/dismiss methods

- (void)showProgress:(float)progress status:(NSString*)string maskType:(FDLoadingGifHUDMaskType)hudMaskType {
    ...
    if(self.maskType != FDLoadingGifHUDMaskTypeNone) {
...
- (void)dismiss {
    NSDictionary *notificationUserInfo = (self.stringLabel.text ? @{FDLoadingGifHUDStatusUserInfoKey : self.stringLabel.text} : nil);
    [[NSNotificationCenter defaultCenter] postNotificationName:FDLoadingGifHUDWillDisappearNotification
                                                        object:nil
    ...
    [UIView animateWithDuration:0.15
                     ...
                     completion:^(BOOL finished){
                         if(weakSelf.alpha == 0) {
                             ...
                             [[NSNotificationCenter defaultCenter] postNotificationName:FDLoadingGifHUDDidDisappearNotification
                                                                                 object:nil
                                                                               userInfo:notificationUserInfo];
...
- (CAShapeLayer *)ringLayer {
    if(!_ringLayer) {
        ...
        _ringLayer = [self createRingLayerWithCenter:center radius:FDLoadingGifHUDRingRadius lineWidth:FDLoadingGifHUDRingThickness color:[UIColor whiteColor]];
...
- (CAShapeLayer *)backgroundRingLayer {
    if(!_backgroundRingLayer) {
        CGPoint center = CGPointMake(CGRectGetWidth(hudView.frame)/2, CGRectGetHeight(hudView.frame)/2);
        _backgroundRingLayer = [self createRingLayerWithCenter:center radius:FDLoadingGifHUDRingRadius lineWidth:FDLoadingGifHUDRingThickness color:[UIColor darkGrayColor]];
...
- (UIImage *)hudSuccessImage {
    ...
    return [UIImage imageNamed:@"FDLoadingGifHUD.bundle/success.png"];
}
...
- (UIImage *)hudErrorImage {
    ...
    return [UIImage imageNamed:@"FDLoadingGifHUD.bundle/loading.gif"];
}
