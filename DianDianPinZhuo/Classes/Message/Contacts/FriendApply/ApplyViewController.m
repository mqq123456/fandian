    ...
    if(self.dataSource.count > indexPath.row)
    {
        ...
        if (entity) {
            ...
            if (applyStyle == ApplyStyleGroupInvitation) {
                ...
                cell.headerImageView.image = [UIImage imageNamed:@"list_ico_dclsx_nor"];
            }
            else if (applyStyle == ApplyStyleJoinGroup)
            {
                ...
                NSArray *messageArray = [entity.reason componentsSeparatedByString:@":;"];
                cell.titleLabel.text = messageArray[1];
                
                [cell.headerImageView sd_setImageWithURL:[NSURL URLWithString:messageArray[2]] placeholderImage:[UIImage imageNamed:@"placeholder"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                    cell.headerImageView.image = image;
                }];
                cell.contentLabel.text = messageArray[0];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ApplyEntity *entity = [self.dataSource objectAtIndex:indexPath.row];
    userprofile.kid = entity.applicantUsername;
        ...
        if (applyUsername && applyUsername.length > 0) {
            for (int i = ((int)[self.dataSource count] - 1); i >= 0; i--) {
                ...
                if (oldStyle == style && [applyUsername isEqualToString:oldEntity.applicantUsername]) {
                    ...
                    oldEntity.applicantNick = [dictionary objectForKey:@"nickname"];
            ...
            newEntity.applicantNick = [dictionary objectForKey:@"nickname"];
