...
- (void)setMessageModel:(MessageModel *)messageModel
{
    ...
    if ([messageModel.username isEqualToString:@"kefu1"]) {// 如果是客服,占位图为客服头像
        placeholderImage = [UIImage imageNamed:@"message_love"];
    }else{// 默认是占位图
        placeholderImage = [UIImage imageNamed:@"status_people_placehold"];
    }
    [self.headImageView sd_setImageWithURL:_messageModel.headImageURL placeholderImage:placeholderImage completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
