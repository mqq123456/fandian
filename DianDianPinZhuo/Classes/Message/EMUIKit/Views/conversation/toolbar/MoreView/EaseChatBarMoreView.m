...
- (void)setupSubviewsForType:(EMChatToolbarType)type
{
    ...
    [_locationButton setFrame:CGRectMake(insets, 10, CHAT_BUTTON_SIZE , CHAT_BUTTON_SIZE)];
    [_locationButton setImage:[UIImage imageNamed:@"chatBar_colorMore_pinzhuo"] forState:UIControlStateNormal];
    [_locationButton setImage:[UIImage imageNamed:@"chatBar_colorMore_pinzhuosel"] forState:UIControlStateHighlighted];
    [_locationButton addTarget:self action:@selector(locationAction) forControlEvents:UIControlEventTouchUpInside];
    _locationButton.tag = MOREVIEW_BUTTON_TAG + 1;
    [_scrollview addSubview:_locationButton];
    
    [_scrollview addSubview:[self createlabel:_locationButton title:@"发起拼桌"]];
    //图片
    _photoButton =[UIButton buttonWithType:UIButtonTypeCustom];
    [_photoButton setFrame:CGRectMake(insets * 3 + CHAT_BUTTON_SIZE*2, 10, CHAT_BUTTON_SIZE , CHAT_BUTTON_SIZE)];
    ...
    [_scrollview addSubview:[self createlabel:_photoButton title:@"照片"]];
    ...
    [_scrollview addSubview:[self createlabel:_takePicButton title:@"拍照"]];
    ///游戏
    ...
    [_audioCallButton setImage:[UIImage imageNamed:@"chatBar_colorMore_game"] forState:UIControlStateNormal];
    [_audioCallButton setImage:[UIImage imageNamed:@"chatBar_colorMore_gameSelected"] forState:UIControlStateHighlighted];
    [_audioCallButton addTarget:self action:@selector(takeAudioCallAction) forControlEvents:UIControlEventTouchUpInside];
    _audioCallButton.tag = MOREVIEW_BUTTON_TAG + 3;
    [_scrollview addSubview:_audioCallButton];

    [_scrollview addSubview:[self createlabel:_audioCallButton title:@"摇骰子"]];
...
- (UILabel *)createlabel:(UIButton *)button title:(NSString *)title{
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(button.x-10, CGRectGetMaxY(button.frame)+10,button.width+20, 20)];
    label.text = title;
    label.textColor = [FDUtils colorWithHexString:@"#666666"];
