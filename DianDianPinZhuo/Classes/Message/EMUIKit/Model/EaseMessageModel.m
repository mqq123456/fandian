...
//        NSString *login = [HQDefaultTool getNickName];
//        NSString *kid = [HQDefaultTool getKid];
        
        NSString *loginUsername = [userInfo objectForKey:kSDKUsername];
#pragma mark－－－修改用户名
        _nickname = (message.messageType == eMessageTypeChat) ? message.ext[@"userName"] : message.ext[@"userName"];
        _isSender = [loginUsername isEqualToString:message.from] ? YES : NO;
        _avatarURLPath = message.ext[@"icon"];
