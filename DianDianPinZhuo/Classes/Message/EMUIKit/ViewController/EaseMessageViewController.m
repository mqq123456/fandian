...
@interface EaseMessageViewController ()<EaseMessageCellDelegate,UIAlertViewDelegate>
{
    UIMenuItem *_copyMenuItem;
    UIMenuItem *_deleteMenuItem;
    UITapGestureRecognizer *_lpgr;
    BOOL showNotice;
...
//    
//    _lpgr = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleLongPress:)];
...
//    [self.tableView addGestureRecognizer:_lpgr];
    ...
    switch ([self.type integerValue]) {
        ...
        case 5:
            ...
            break;
        case 6:
            self.webTitle = self.share_hint;
            break;
        case 7:
            self.webTitle = self.content;
            break;
        default:
            break;
    }
    ...
    //单聊不发送默认消息
        if (self.share) {
             ...
             *  先弹出一个框
             */
            UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"分享给饭友？" message:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"分享", nil];
            alertView.tag = 888888;
            [alertView show];

        }
        
    }else{
        if ([[FDGroupDetailTool sharedInstance] isExistRecordWithGroupJoined:self.conversation.chatter]) {///加入过
            showNotice = NO;
            if (self.share) {
                
                UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"分享到群组？" message:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"分享", nil];
                alertView.tag = 999999;
                [alertView show];
            }
            ...
            if (self.is_join_in) {
                ...
                //self.content =[NSString stringWithFormat:@"%@加入群组，给各位问好。",[HQDefaultTool getNickName]];
//                if (!self.share_hint ||[self.share_hint isEqualToString:@""]) {
//                    self.share_hint = @"查看资料";
//                    
//                }
                [self sendTextMessage:self.content withExt:@{@"userName":[HQDefaultTool getNickName],@"icon":[HQDefaultTool getHead],@"type":@"1",@"content":[NSString stringWithFormat:@"%@加入群组，给各位问好。",[HQDefaultTool getNickName]],@"confirm":@"查看资料",@"webTitle":self.webTitle,@"webUrl":self.webUrl}];
            }
           
            NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:self.conversation.chatter,@"group_id",[HQDefaultTool getKid],@"kid", nil];
            [[FDGroupDetailTool sharedInstance] addGroupJoined:dic group_idString:self.conversation.chatter];
            if (self.share) {
                UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"分享到群组？" message:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"分享", nil];
                alertView.tag = 777777;
                [alertView show];
        ...
        [exportSession exportAsynchronouslyWithCompletionHandler:^{
            switch ([exportSession status]) {
                case AVAssetExportSessionStatusFailed: {
                    MQQLog(@"failed, error:%@.", exportSession.error);
                } break;
...
- (void)_textMessageCellSelected:(id<IMessageModel>)model{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row+1<self.dataArray.count) {
        object1 = [self.dataArray objectAtIndex:indexPath.row+1];
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if (object1) {
            model = object1;
            NSString *kid;
            if (model.message.groupSenderName.length == 32) {
                kid = model.message.groupSenderName;
            }else{
                if (model.message.from.length==32) {
                    kid = model.message.from;
                }else{
                    kid = model.message.to;
                }
            }

            if ([model.message.ext[@"type"] isEqualToString:@"1"]&&showNotice&&[[HQDefaultTool getKid]isEqualToString:kid]) {
                timeCell.title_bgView.hidden = NO;
                timeCell.title = [HQDefaultTool getGroups_notice];
            }else{
                timeCell.title_bgView.hidden = YES;
                timeCell.title = [NSString stringWithFormat:@"%@",object];
            }
        ...
        CGRect contentRect = [[NSString stringWithFormat:@"%@",object] boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width, 100) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16]} context:nil];
        [timeCell addConstraint:[NSLayoutConstraint constraintWithItem:timeCell.titleLabel attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationGreaterThanOrEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:contentRect.size.width]];
        [timeCell layoutIfNeeded];
        return timeCell;
    }
    else{
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if (object1) {
            ...
            if ([model.message.ext[@"type"] isEqualToString:@"1"]&&showNotice&&[[HQDefaultTool getKid]isEqualToString:kid]) {
                CGRect contentRect = [[HQDefaultTool getGroups_notice] boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width-140, 1000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]} context:nil];
                return contentRect.size.height+20;
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeMovie]) {
        ...
        if ([fileman fileExistsAtPath:videoURL.path]) {
            ...
            if (error) {
                MQQLog(@"failed to remove file, error:%@.", error);
            }
        }
...
- (void)messageCellSelected:(id<IMessageModel>)model
{
    ...
    switch (model.bodyType) {
        ...
        {
            [self _textMessageCellSelected:model];
        }
            break;
...
- (void)didStartRecordingVoiceAction:(UIView *)recordView
{
    ...
    if ([self _canRecord]) {
        ...
        [[EMCDDeviceManager sharedInstance] asyncStartRecordingWithFileName:fileName completion:^(NSError *error)
         {
             if (error) {
                 MQQLog(NSLocalizedString(@"message.startRecordFail", @"failure to start recording"));
             }
         }];
    }
}
...
//    [self.navigationController pushViewController:locationController animated:YES];
    ...
    FDSponsorTopicViewController *sponsorTopicVC = [[FDSponsorTopicViewController alloc]init];
    [self.navigationController pushViewController:sponsorTopicVC animated:YES];
...
//    UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"功能开发中，敬请期待" message:@"" delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
//    alertView.tag = 888;
//    [alertView show];
                         ...
                         andAddress:(NSString *)address
{
    EMMessage *message = [EaseSDKHelper sendLocationMessageWithLatitude:latitude
                                                          ...
                                                          messageType:[self _messageTypeFromConversationType]
                                                    requireEncryption:NO
                                                           messageExt:@{@"userName":[HQDefaultTool getNickName],@"icon":[HQDefaultTool getHead],@"type":@"0",@"content":@"",@"confirm":@"",@"webTitle":@"",@"webUrl":@""}];
    [self addMessageToDataSource:message
                        progress:nil];
}
 ...
 */
- (void)shareToSingal{
    [self sendTextMessage:self.content withExt:@{@"userName":[HQDefaultTool getNickName],@"icon":[HQDefaultTool getHead],@"type":self.type,@"content":self.content,@"confirm":self.share_hint,@"webTitle":self.webTitle,@"webUrl":self.webUrl}];
